...
<https://neb-x-357243228666.asia-southeast1.run.app>

Hosted on Cloud Run, no sign-in. Nothing to install: open it, upload a file, read the
assessment. The sections below are for running it locally instead.

## Quick start

```bash
git clone https://github.com/nebulax-group/neb-x.git && cd neb-x
./scripts/train.sh                 # Windows: scripts\train.bat
./scripts/app.sh                   # Windows: scripts\app.bat
```

That is the whole setup. The script creates `.venv`, installs `scripts/requirements.txt` and
opens the app in a browser. If it will not start, `./scripts/check.sh` says what the machine
is missing.

Every command below is a script, written in its macOS/Linux form. On Windows, run the
`.bat` sitting beside it.
...
Fits every subsystem that has a trainer and reports the ones that do not. Or one at a time:

| Subsystem | Script | |
|---|---|---|
| Door | `./scripts/train_door.sh` | CV accuracy 1.0000 over 110 cycles |
| ACV | — | fits no parameters, so nothing to train. Read `src.acv.report.confirmation` before trusting a ranking |
| Rail Corrugation | `./scripts/train_rail.sh` | macro F1 0.750 ± 0.114, 0.757 pooled. ~60 s, or ~390 s the first time on a machine. Pass `--refresh` after editing `features.py` |
| SHM | `./scripts/train_shm.sh` | LOO MAPE 2.52%, score 0.9748 |

Training needs `data/`, which is not in git (~6 GB): copy the organisers' `PS3/02_Datasets/` in
using the mapping in [data/README.md](data/README.md). Checkpoints land in
`outputs/models/<subsystem>/`, which is gitignored, so every machine trains its own.

## Predictions and the submission

```bash
./submit.sh
```

Runs the held-out test inputs through every subsystem, validates each CSV against the schema,
and builds `<Team Name>/` at the repository root. That folder is the upload. It does not train,
so run `./scripts/train.sh` first on whatever machine builds it.

The three steps also run alone: `./scripts/generate.sh`, `./scripts/validate.sh`,
`./scripts/package.sh`.
...
Builds the root `Dockerfile` with Cloud Build and serves it on Cloud Run, then prints the
URL. Redeploying keeps the same URL. `.gcloudignore` exists because gcloud would otherwise
fall back to `.gitignore` and strip `outputs/models/`, shipping an app with no checkpoints.
Override the defaults with `SERVICE=` and `REGION=`.
...
├── scripts/                      thin entry-point wrappers, install.sh, requirements.txt
├── video/                        manim scene sources — see video/README.md
├── outputs/                      models, predictions, plots, logs (gitignored, regenerable)
└── <Team Name>/                  the folder we send, built by ./submit.sh (gitignored)
```
...
CSVs but does **not** train, so run `./scripts/train.sh` first on whatever machine builds the
submission.
