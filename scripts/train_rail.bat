@echo off
REM Thin wrapper: fit the rail corrugation classifier, creating or refreshing .venv first.
REM See src\rail\train.py.
setlocal
cd /d "%~dp0.."
...
:run
echo.
REM Pass --refresh through to re-extract the feature cache, which is mandatory after
REM editing features.py; the module owns that default, not this script. Expect ~390 s
REM with a refresh and ~230 s without one.
"%VENV_PY%" -m src.rail.train %*
exit /b %errorlevel%
