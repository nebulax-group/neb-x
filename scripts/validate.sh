#!/usr/bin/env bash
# Thin wrapper: check the prediction CSVs, creating or refreshing .venv first.
# See src/submission/validate.py.
set -euo pipefail
...
echo
# No arguments sweeps every subsystem and skips the ones nobody has produced yet;
# a path checks that one file. The module decides, not this script.
exec python -m src.submission.validate "$@"
