@echo off
REM Thin wrapper: build the submission folder, creating or refreshing .venv first.
REM See src\submission\package.py.
setlocal
cd /d "%~dp0.."
...
:run
echo.
REM Pass --team "Your Name" through; the module decides the layout, not this script.
"%VENV_PY%" -m src.submission.package %*
exit /b %errorlevel%
